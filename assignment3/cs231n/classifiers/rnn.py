...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        word_vec, we_cache = word_embedding_forward(captions_in, W_embed)
        ...
        grads['W_embed'] = word_embedding_backward(dword_vec, we_cache)
        dfeatures, grads['W_proj'], grads['b_proj'] = affine_backward(dh0, a_cache)
    ...
    def sample(self, features, max_length=30):
        ...
        prev_words = (self._start * np.ones(N)).astype(np.int)
        prev_h,_ = affine_forward(features, W_proj, b_proj)
        if self.cell_type=='lstm':
            prev_c = np.zeros_like(prev_h)
        for t in range(max_length):
            x = W_embed[prev_words]
            # x.shape = (N,W)
            if self.cell_type=='rnn':
